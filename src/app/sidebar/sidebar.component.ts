...
export class SidebarComponent implements OnInit {
  ...
  //       conversation.lastMessage?.body?.toLowerCase()?.includes(this.searchText?.toLowerCase())
  //     );
  //   });
  // }

  get filteredConversations() {
    const searchText = this.searchText || ''; // Use an empty string if searchText is undefined
    return this.conversations.filter((conversation: any) => {
      return (
        conversation.name?.toLowerCase()?.includes(searchText.toLowerCase()) ||
        conversation.lastMessage?.body?.toLowerCase()?.includes(searchText.toLowerCase())
      );
    });
  }
  ...
  ngOnInit(): void {
    this.dataService.getAllChatList().subscribe((res:any)=>{
